...
quats_21 = rot.quatOfExpMap(grain_data['scan21'][:,3:6].T)
...
for i in range(len(no_load_scans)):

    q108 =rot.quatOfExpMap(grain_data['scan%d'%no_load_scans[i]][:,3:6].T)
    expmaps_grain.append(grain_data['scan%d'%no_load_scans[i]][grain,3:6])
    mis = rot.misorientation(quats_21[:,grain:grain+1], q108[:,grain:grain+1],(crystal_symmetry,))[0]
    mis_ang.append(mis)
...
for i in range(len(pts)):
    ax2.plot(pts[i, 0], pts[i, 1], 'o',color = cmaplist[unloaded_colornums[i]])
...
plt.text(0.65,0.49,r'(10$\bar{1}$0)',fontsize=16)
# plt.text(,r'(2$\bar{1}$$\bar{1}$0)',fontsize=16)
plt.text(0.9,-0.08,r'(2$\bar{1}$$\bar{1}$0)',fontsize=16)
...
for i in range(len(pts)):
    ax_in.plot(pts[i, 0], pts[i, 1], 'o',color = cmaplist[unloaded_colornums[i]])
